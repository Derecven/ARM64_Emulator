...
void executeAND(string op1, string op2, string op3) {
    x_registers[op1] = x_registers[op2] & x_registers[op3];//Takes the value inside register op2 and op3 and performs bitwise operation, stores resutls in op1.
}
//Execute ADD instruction
void executeADD(string op1, string op2, string op3) {
    //Assuming operands are in the form {"ADD","xd","x1","x2"}
    x_registers[op1] = x_registers[op2] + x_registers[op3];//Takes the value inside register op2 and op3 and add them together, stores resutls in op1.
}
...
void executeB(string destination) {
    ...
    pc_register = destination;//updates program counter to the new destination
}
//Execute B.GET instruction (branch if greater or equal to) 
void executeBGET(string destination, string op1, string op2) {
    //Assuming operands are in the form {"B","target_address"}
    int operand1 = x_registers[op1]; //storing value from register op1
    int operand2 = stoi(op2); //assumes op2 is the immidiate value converts string to integer.
    
    //checks conditon which is if it is greater than or equal
    if (operand1 >= operand2) {
        cout << "B.GET: Branch to: " << destination << "\n";
        pc_register = destination;//updates  program counter to new destination
    }
    else {
...
void executeBLE(string destination, string op1, string op2) {
    //Assuming operands are in the form {"B","target_address"}
    int operand1 = x_registers[op1]; //storing value from register op1
    int operand2 = atoi(op2); //assumes op2 is the immidiate value converts string to integer

    //checks conditon which is if it is less than or equal to
    if (operand1 <= operand2) {
        cout << "B.GET: Branch to: " << destination << "\n";//branch to destination
        pc_register = destination;//update program counter to destination.
    }
    else {
...
void executeCMP(string op1, string op2) {
    ...
    int nFlag = 0;
    int operand1 = x_registers[op1]; //storing value from register op1
    int operand2 = x_registers[op2];//storing value from register op2
...
void executeEOR(string op1, string op2, string op3) {
    register[op1] = x_registers[op1] ^ x_registers[op2]; //Takes value from op2 & op3, performs XOR bitwise op and stores results in op1.

}
//Execute LDR instruction
void executeLDR(string op1, string op2) {
    int address = x_registers[op2];//storing the value stored in op2 inside of address
    register[op1] = memory[address]; //assiging that memory address to the specified target register which is op1.
}
...
void executeLDRB(string op1, string baseReg, int regOffset) {
    int address = x_registers[baseReg] + regOffset; //Takes the base registers mem address  and adds the offset to calculate the new mem address
    register[op1] = memory[address]; //the byte is loaded from mem address to the location of new destination.
}
//Execute MOV instruction
void executeMOV(string destination, string op1) {
    int operand1 = x_registers[op1]; //storing the value store in op1 inside of operand 1
...
void executeMUL(string op1, string op2, string op3) {
    register[op1] = x_registers[op2] * x_registers[op3]; //Takes values from op2 and op3, multiplies them and stores results in register op1.
}
...
void executeSUB(string op1, string op2, string op3) {
        register[op1] = x_registers[op2] - x_registers[op3];//Takes value from op2 and op3 and stores the value in op1.
    }
    //Execute STR instruction
void executeSTR(string op1, string op2) {
    int address = x_registers[op2]; // int address is assigned the value from op 2 
    memory[address] = x_registers[op1]; //this assigns value in op2 to the memeory address
    }
    //Execute STRB instruction
void executeSTRB(string op1, string baseReg, int regOffset) {
    int address = x_registers[baseReg] + regOffset; //Takes the base registers mem address  and adds the offset to calculate the new mem address
    memory[address] = x_registers[op1] & 0x0f; //the stores the lower 8 bits from the source register into memory
    }
